...
Defines whenever the k8up database schedule is enabled or not.
Disabled by default.
...
Helpful to see the state of the last jobs, but too many jobs causing overhead in terms of the metadata needs to be kept in K8s.
...
Name of the secret created in the namespace to store the K8up repository password.
...
[horizontal]
type:: string
default:: `?{vaultkv:${customer:name}/${cluster:name}/keycloak/k8up-repo-password}`

Vault reference to the K8up repository password.
...
Name of the secret created in the namespace to store the K8up S3 connection credentials.
...
Bucket name where the backups gets stored.
...
[horizontal]
type:: string
default:: `?{vaultkv:${customer:name}/${cluster:name}/keycloak/k8up-s3-accesskey}`
...
[horizontal]
type:: string
default:: `?{vaultkv:${customer:name}/${cluster:name}/keycloak/k8up-s3-secretkey}`
